...
class EvalHarness:
    ...
    def _check_stalled_detection(self, variant_id: str) -> tuple[float, str]:
        ...
        try:
            data = json.loads(Config.PROJECT_STATUS_PATH.read_text(encoding="utf-8"))
            projects = data.get("projects", [])
            if not projects:
    ...
    def _check_no_stale_workers(self, variant_id: str) -> tuple[float, str]:
        ...
        try:
            data = json.loads(Config.WORKER_REGISTRY_PATH.read_text(encoding="utf-8"))
    ...
    def _check_soul_matches_variant(self, variant_id: str) -> tuple[float, str]:
        ...
        if not Config.SOUL_MD_PATH.exists():
            ...
            content = Config.SOUL_MD_PATH.read_text(encoding="utf-8")[:200]
    ...
    def _check_last_cycle_complete(self, variant_id: str) -> tuple[float, str]:
        ...
        try:
            data = json.loads(Config.ACTIVE_MISSION_PATH.read_text(encoding="utf-8"))
    ...
    def _check_no_secrets_in_memory(self, variant_id: str) -> tuple[float, str]:
        ...
        mgr = SecretsManager()
        try:
            content = Config.TRADER_MEMORY_PATH.read_text(encoding="utf-8")
